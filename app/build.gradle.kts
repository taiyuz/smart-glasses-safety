    ...
    doLast {
        ...
        try {
            ...
            if (actual != efficientDetSha256) {
                outFile.delete()
                logger.warn("EfficientDet-Lite0 SHA-256 mismatch (got $actual). ML Kit remains default.")
            }
        } catch (e: Exception) {
            logger.warn("Could not download EfficientDet-Lite0 (${e.message}). ML Kit remains default.")
        }
    }
}
